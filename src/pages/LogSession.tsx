...
export default function LogSession({
  ...
  const deleteCustomTag = (tag: string) => {
    ...
    toast({ title: "Tag deleted", description: `"${tag}" has been removed from your custom tags` });
  };
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (!finalSubject || !date || !goalTopic) {
      toast({ title: "Missing fields", description: "Subject, date, and topic are required", variant: "destructive" });
      return;
    }
    ...
    if (isNaN(hoursNum) || hoursNum <= 0) {
      ...
      if (hoursNum <= 0) {
        toast({ title: "Invalid time range", description: "Please enter valid start and end times", variant: "destructive" });
        return;
      }
    }
